...
def save_forecast_timeseries(tms, time_series_data, model_date_time, opts):
    ...
    for day in days_list:
        ...
        if type != 'Error':
            ...
            event_id = tms.get_timeseries_id(run_name=meta_data['run_name'],
                                  station_name=meta_data['station'],
...
def save_forecast_timeseries_data(tms, time_series_data, run_date, opts):
    df = pd.pivot_table(time_series_data, columns=time_series_data['time'].str[:10])
    days_list = df.columns.values
    for day in days_list:
...
def upload_data_to_db(db, run_datetime, discharge_file, run_name, force_insert=False):
    try:
        tms = Timeseries(db)
        ...
        save_forecast_timeseries(tms, time_series_data, run_datetime, opts)
...
def upload_discharge_data(db, run_date, discharge_file, run_name, force_insert=False):
    try:
        tms = Timeseries(db)
        CSV_NUM_METADATA_LINES = 2
        DAT_WIDTH = 12
        ...
        save_forecast_timeseries(tms, time_series_data, run_date, opts)
